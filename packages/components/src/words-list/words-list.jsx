...
function WordsList(props) {
  ...
  const sayExamples = useCallback(async (currentWord) => {
    await new Promise((resolve) => {
      setTimeout(async () => {
        await api.audio.voice(currentWord.example, currentWord.exampleTranslation);
        resolve();
      }, 300);
    });
  }, []);
  ...
  const sayWord = useCallback(
    async (currentWord, index, repeatTime) => {
      ...
      await api.audio.voice(currentWord.english, currentWord.russian, delayBetweenTranslates);
